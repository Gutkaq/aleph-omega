...
data Config = Config [(Direction, Integer)] deriving (Show, Eq, Generic)
zeroConfig :: Config
zeroConfig = Config []
...
iota 0 (k0@(KInf0 _)) = KInf1 deltaC
...
isRadiativelyClosed family = not (null family) && null (radiativeClosure family \\ family)
  where xs \\ ys = filter (`notElem` ys) xs
...
satisfiesAxioms (AOF cell iotaFn piFn _ _) =
  let n = levelOf cell
      embedded = iotaFn n cell
      projected = piFn (n + 1) embedded
  in cell == projected
